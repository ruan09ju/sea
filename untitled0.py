...
Original file is located at
    https://colab.research.google.com/drive/19g99PB895ZIY5LP6v9UxA0ixoY46pH_t
"""
...
try:
    # 🎯 完美對齊您的設定：讀取名稱為 'work' 的金鑰
    api_key = userdata.get('work')
    if api_key:
        os.environ["GEMINI_API_KEY"] = api_key
        print("✅ 成功自動讀取 Colab 密鑰 'work'！")
except Exception as e:
    print(f"⚠️ 提示：讀取密鑰 'work' 失敗，請確認左側鑰匙中有新增此名稱。錯誤：{e}")
...
# 🎯 自動化核心：直接從系統或 Colab Secrets 抓取您設定好的 'work' 金鑰值
try:
    AUTOMATIC_KEY = userdata.get('work')
except:
    AUTOMATIC_KEY = os.environ.get("GEMINI_API_KEY", "")
...
# 將自動讀取的標準金鑰強制寫入 Streamlit 的環境中
os.environ["GEMINI_API_KEY"] = "{AUTOMATIC_KEY}"

# 📦 靜態經典精選題目庫 (擴充至 15 題完整版)
PRESET_PUZZLES = {{
    "【精選題庫 01】海龜肉湯": {{
        "title": "【精選題庫 01】海龜肉湯",
...
<style>
    ...
    /* ✨ 新增下面這行 CSS：精準將側邊欄所有元件的說明文字標籤（包含 Selectbox）強制變成純白色 */
...
# 指向雲端硬碟資料夾
HISTORY_DIR = "/content/drive/MyDrive/TurtleSoup_Records"
...
print("✅ app.py 構建成功！(已成功將『從精選題庫挑選案件』文字優化為高對比白色！)")

# ==================== 第三段：清理與發動 ====================
import os
import time

# 1. 強制殺死之前的殘留程序，釋放通訊埠 8501
os.system("pkill -9 -f streamlit")
time.sleep(1)
print("🧹 [系統提示] 已成功清理背景殘留程序。")

# 2. 安全建立雲端目錄
DRIVE_DIR = "/content/drive/MyDrive/TurtleSoup_Records"
